...
class InteractionBlockPrompt(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 ...
                 deform_ratio=1.0, extra_extractor=False, with_cp=False, next_dim=64):
        super().__init__()
        ...
        if self.next_dim is not None:
            self.up_layer1 = nn.Conv2d(dim, self.next_dim, kernel_size=3, stride=2, padding=1, bias=False)
            self.up_layer2 = nn.Conv2d(dim, self.next_dim, kernel_size=3, stride=2, padding=1, bias=False)
            self.up_layer3 = nn.Conv2d(dim, self.next_dim, kernel_size=3, stride=2, padding=1, bias=False)
    ...
    def forward(self, x, c1, c2, c3, stage, hw_shape, backbone='swin'):
