...
from src.data.imagenet import get_transforms
...
def main():
    ...
    loader = data_loader(Path(args.ds_val), args.image_size)

    d = {}

    if args.pretrained is None or args.pretrained.endswith('.ckpt'):
        print('=== Use checkpoint ===')
        net = get_model(args)
        cast_amp(net, args)
        if args.pretrained:
            ms.load_checkpoint(str(args.pretrained), net=net)
        print(
    ...
    with open(args.pred_output, 'w') as f:
