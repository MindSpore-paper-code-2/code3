...
if [ $# -lt 2 ]
then
    echo "Usage: bash ./scripts/run_distributed_train_gpu.sh N_GPU CONFIG_PATH DEVICE_IDS EXTRA_ARGS"
...
mpirun -n $N_GPU --allow-run-as-root --output-filename log_output \
  python train.py --device_id $DEVICE_IDS --device_target GPU \
  --tnt_config "$CONFIG_PATH" \
  --dir_ckpt "$DIR_CKPT" \
  --dir_best_ckpt "$DIR_CKPT/best/" \
  --dir_summary "$DIR_SUMMARY" $EXTRA_ARGS \
    > log 2>&1 &
cd ..
