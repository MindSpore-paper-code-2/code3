...
def get_model(
    ...
    models = {
        't2t_vit_7': t2t_vit_7,
        't2t_vit_10': t2t_vit_10,
        't2t_vit_12': t2t_vit_12,
        't2t_vit_14': t2t_vit_14,
        't2t_vit_19': t2t_vit_19,
        't2t_vit_24': t2t_vit_24,
        't2t_vit_t_14': t2t_vit_t_14,
        't2t_vit_t_19': t2t_vit_t_19,
...
def correct_layer_name(layer_name: str) -> str:
    """Correct layer names for models from old version of code."""
    if 'tokens_to_token.attention' not in layer_name:
        return layer_name

    old2new = {'.mlp.0': '.mlp.fc1',
               '.mlp.2': '.mlp.fc2'}
    for k, v in old2new.items():
        layer_name = layer_name.replace(k, v)
    return layer_name
    ...
    if os.path.isfile(args.pretrained):
        ...
        for key, value in param_dict.copy().items():
            if 'head' in key:
                if value.shape[0] != args.num_classes:
                    ...
                    param_dict.pop(key)
                    continue
            new_param_dict[correct_layer_name(key)] = value
        if exclude_epoch_state:
            ...
            for state_param in state_params:
                if state_param in new_param_dict:
                    new_param_dict.pop(state_param)
