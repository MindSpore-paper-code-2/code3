...
def _rotate_level_to_arg(level, hparams):
    """_randomly_negate"""
    _ = hparams  # for signature compatibility
    # range [-30, 30]
...
def _enhance_level_to_arg(level, hparams):
    ...
    _ = hparams  # for signature compatibility
    # range [0.1, 1.9]
    return (level / _MAX_LEVEL) * 1.8 + 0.1,  # noqa
...
def _posterize_level_to_arg(level, hparams):
    ...
    return int((level / _MAX_LEVEL) * 4),  # noqa
...
def _posterize_increasing_level_to_arg(level, hparams):
    ...
    return 4 - _posterize_level_to_arg(level, hparams)[0],  # noqa
...
def _posterize_original_level_to_arg(level, hparams):
    ...
    # intensity/severity of augmentation decreases with level
    return int((level / _MAX_LEVEL) * 4) + 4,  # noqa
...
def _solarize_level_to_arg(level, hparams):
    ...
    # range [0, 256]
    # intensity/severity of augmentation decreases with level
    return int((level / _MAX_LEVEL) * 256),  # noqa
...
def _solarize_increasing_level_to_arg(level, hparams):
    ...
    # range [0, 256]
    # intensity/severity of augmentation increases with level
    return 256 - _solarize_level_to_arg(level, hparams)[0],  # noqa
...
def _solarize_add_level_to_arg(level, hparams):
    ...
    _ = hparams  # for signature compatibility
    # range [0, 110]
    return int((level / _MAX_LEVEL) * 110),  # noqa
...
class AugmentOp:
    ...
    def __init__(self, name, prob=0.5, magnitude=10, hparams=None):
        hparams = hparams or HPARAMS_DEFAULT
