...
class TwoWayTransformer(nn.Cell):
    ...
    def construct(
        ...
        """
        Args:
          image_embedding (mindspore.Tensor): image to attend to. Should be shape
            B x embedding_dim x h x w for any h and w.
          image_pe (mindspore.Tensor): the positional encoding to add to the image. Must
            have the same shape as image_embedding.
          point_embedding (mindspore.Tensor): the embedding to add to the query points.
            Must have shape B x N_points x embedding_dim for any N_points.

        Returns:
          mindspore.Tensor: the processed point_embedding
          mindspore.Tensor: the processed image_embedding
        """
...
class Attention(nn.Cell):
    ...
    def construct(self, q: Tensor, k: Tensor, v: Tensor, size: Tensor = None) -> Tensor:
        # Input projections
        q = self.q_proj(q)
        k = self.k_proj(k)
        v = self.v_proj(v)
